...
class GameLogicTask6(SpriteNumber: Int, BackTileNumber: Int) extends Module {
  ...
  io.spriteVisible(9) := (bubbleYReg > 90.S).asBool
  ...
  switch(stateReg) {
    ...
    is(compute1) {
      ...
      bubbleYReg := Mux(bubbleYReg > 0.S, bubbleYReg - 1.S, 384.S)
